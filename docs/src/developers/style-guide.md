...
!!! info
    The style guide is always a work in progress, and not all ExpressGPT code
    follows the rules. When modifying ExpressGPT, please fix the style violations
...
ExpressGPT uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/ExpressGPT.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the ExpressGPT directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to ExpressGPT have
