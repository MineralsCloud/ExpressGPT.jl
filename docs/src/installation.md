...
[ExpressGPT](https://github.com/MineralsCloud/ExpressGPT.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("ExpressGPT")
   ```

3. Run

   ```julia-repl
   julia> using ExpressGPT
   ```
...
version of ExpressGPT, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/ExpressGPT.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/ExpressGPT.jl)
for new releases.
Once we release a new version, you can update ExpressGPT by typing

```@repl
using Pkg
Pkg.update("ExpressGPT")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall ExpressGPT
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("ExpressGPT")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall ExpressGPT.
